namespace DavidKuehn.Portfolio.WebApi.Services.IdentityProvider
{
    ...
    namespace Core_RBS_Tokens.Services
    {
        ...
        public class SecurityServices
        {
            ...
            public SecurityServices(UserManager<IdentityUser> userManager, 
            ...
            public async Task<SecureResponse> RegisterUserAsync(RegisterUser user)
            {
                ...
                {
                    response.StatusCode = 400;
                    response.Message = "User name is not provided";
                }
                else
                {
                    ...
                    {
                        response.StatusCode = 400;
                        response.Message = $"User {user.UserName} is already exist";
                    }
                    else
                    {
                        // Create user
                        ...
                        if (result.Succeeded)
                        {
                            ...
                            response.Message = $"User {user.UserName} is created successfully";
                        }
                        else
                        {
            ...
            public async Task<SecureResponse> AuthenticateUser(LoginUser user)
            {
                SecureResponse response = new SecureResponse();
                if (user == null)
                {
                ...
                else
                {
                    ...
                    if (identityUser == null)
                    {
                        ...
                        response.Message = $"User {user.UserName} is not present";
                    }
                    else
                    {
                        ...
                        if (roles.Count == 0)
                        {
                            ...
                            response.Message = $"The user {user.UserName} does not belong to any role, ad hence the user cannot access the application";
                        }
                        else
                        {
                            ...
                            if (authStatus.Succeeded)
                            {
                                ...
                                response.Message = $"User {user.UserName} Logged in successfuly";
                            }
                            ...
                            {
                                response.StatusCode = 500;
                                response.Message = $"Error Occurred for User {user.UserName} Login";
                            }
                        }
                    }
                }
